...
export default function Products({ location: { search } }) {
  ...
  const { product, procat, carousel, filterProcat, category } = useSelector(
    (state) => {
      return {
  ...
  const filterCategory = () => {
    return (
      <div>
        <InputLabel style={{ marginBottom: "5px" }}>Filter</InputLabel>
        <Select
          fullWidth
          variant="outlined"
          value={categoryName}
  ...
  const renderSort = () => {
    return (
      <div>
        <InputLabel style={{ marginBottom: "5px", textAlign: "right" }}>
          Urutkan
        </InputLabel>
        <Select
          fullWidth
  ...
  const handleSort = (event) => {
    ...
    search
      ? dispatch(
          getFilterProductCategory(
            search ? search.slice(10) : null,
            event.target.value
          )
        )
      : chip.length !== 0
      ? dispatch(getProductCategory(query, sortBy))
      : dispatch(getProduct("product_details", sortBy));
  };
  ...
  const renderCard = () => {
    return (search ? filterProcat : chip.length !== 0 ? procat : product).map(
      (item) => {
        return (
          <Link
            to={{
            ...
            <Button>
              <Card className={classes.card}>
                ...
                <CardContent>
                  ...
                  <Typography>
                    Rp. {item.price ? item.price.toLocaleString() : null}
                  </Typography>
                </CardContent>
              </Card>
            </Button>
  ...
  return (
    <div className={classes.root}>
      <div
        ...
        style={{
          backgroundImage: `url(${carousel[2] ? carousel[2].image : null})`,
        }}
      ></div>
      <Paper className={classes.paper}>
        ...
        {renderChip()}
        <div className={classes.productCard}>{renderCard()}</div>
      </Paper>
    </div>
  );
}
